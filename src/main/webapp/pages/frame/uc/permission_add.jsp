...
function link(){
	var pername = $("#pn").val();
		if(pername.length>0){
		   alert('保存成功！');
		   document.getElementById("fom").action="pc/addper";
...
<script type="text/javascript">
	$(function () {
		...
		$("#pn").blur(function(){
			var pername = $("#pn").val();
			if(pername.length>0){
				$.post("pc/checkre",{"pername":pername},function(data){
						if(data!="0"){
							 $("#msg").html("权限名称已存在,请换一个!!");
							 $("#btn").hide();
							 $("#bt").hide();
						}else{
							$("#msg").html("可以使用!");
						...
						<legend>添加权限</legend>
					  	<table border="0" cellpadding="2" cellspacing="1" style="width:100%">
					  		<tr>
					    		...
					    		<td width="41%">
					    			<input name="pname" id="pn" class="text" style="width:154px" type="text" size="40" />
				        			<span id="msg" class="red"> *</span>
				        		</td>
					    	</tr>
					  		<tr>
					    		<td nowrap align="right" width="13%">权限路径:</td>
					    		<td width="41%">
					    			<input name="perurl" class="text" style="width:154px" type="text" size="40" />
				        			<span id="msg" class="red"> *</span>
				        		</td>
					    	</tr>
					  		<tr>
					    		<td nowrap align="right" width="13%">父权限名称:</td>
					    		<td width="41%">
						    		<select name="parentid">
				    			        <c:forEach items="${perlist}" var="per">								
											<option value="${per.perid }">
											    ${per.pname }
											</option>
									    </c:forEach>
								    </select>
